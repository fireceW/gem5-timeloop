...
void write_input_maxrix_to_file(float arr[4][4], int dim,  const char *filename){
    FILE* fp;
    fp = fopen(filename, "w");
    for (int s = 0; s < dim; s++) {
        if(s == dim-1 ){
            fprintf(fp, "%f ", arr[dim][s]);
        }else{
            fprintf(fp, "%f, ", arr[dim][s]);
        }
    }
...
void write_weight_maxrix_to_file(float arr[4][4], int dim,  const char *filename){
    FILE* fp;
    fp = fopen(filename, "w");
    for (int s = 0; s < dim; s++) {
        if(s == dim-1 ){
            fprintf(fp, "%f ", arr[s][dim]);
        }else{
            fprintf(fp, "%f, ", arr[s][dim]);
        }
    }
...
int read_tmp_from_file(const char *filename){
    FILE* fp;
    float res;
    fp = fopen("/home/gem5/tmp_result/result.txt", "r");
    fscanf(fp, "%4f", &res);
    fclose(fp);
   // printf("the tmp is %f\n",res);
    return res;
